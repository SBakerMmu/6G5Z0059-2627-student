...
If you are unsure, then create projects under `C:\users\<user>\` where <user> is your Windows username.

> ☑ If you are an advanced Windows user, you might consider creating a dedicated folder for your development projects, for example `C:\_dev\` or similar. You can also create a 'DevDrive' which is a separate partition on your hard drive dedicated to development work. See (Set up a Dev Drive on Windows 11)[https://learn.microsoft.com/en-us/windows/dev-drive/]. It is not necessary to do this, but it may give you a performance benefit.
...
```
For most of the labs, creating classes within the project and writing the test code in the main() method is sufficient.

You can either:

- Create one project for each lab exercise, which gives you an empty project to work in, but you will end up with a lot of projects.
- One project holding all the lab exercise for the week, which, but you will have to manage multiple classes in the same project. We cover Java packages shortly which is a way of organizing classes into separate folders within a single project and using package names so that you can have two classes with the same name in different packages.
- Note that when we get to using Spring, we will be using Spring tooling and Maven to manage our projects. In this case you will need one project per lab exercise.
...
**Question: Why does Java have both Comparable<T> and Comparator<T> interfaces?**.
