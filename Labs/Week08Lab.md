...
In examples of Dependency Injection so far, we have manually constructed the objects and passed them into the constructors of the classes that need them. For example a `CheckoutService` class requires a concrete implementation of the `AbstractCreditCardService` interface to be passed into its constructor.
...
```
We can have multiple implementations of the AbstractCreditCardService interface, for example a TestCreditCardService and a RealCreditCardService.
...
class TestCreditCardService implements AbstractCreditCardService {
    ...
    public void takePayment(String cardNumber, int expiryYear, int expiryMonth, String cv2, double amount) {
        System.out.format("Test Credit Card service charging credit card %s for £%.2f%n", cardNumber, amount);
    }
}
```

The CheckoutService class depends on the AbstractCreditCardService interface and requires a concrete realization of AbstractCreditCardService to be passed into its constructor.
```java
...
```
To make the system work, we need to manually construct a concrete instance of AbstractCreditCardService and pass it into the CheckoutService constructor. This is called "manual dependency injection" and the code is configuring the CheckoutService instance.
...
- **Project**: Maven
- **Language**: Java
- **Spring Boot**: Choose latest stable (at time of writing this was 4.1.0 but will probably be later when you do this lab)
- **Project Metadata**
  - Group: uk.ac.mmu.sda
  - Artifact: lab08
  - Name: lab08
  - Description: Week 08 Lab - Dependency Injection with Spring
  - Package name: uk.ac.mmu.sda.lab08
  - Packaging: Jar
...
> Each Maven project has its own coordinates (groupId, artifactId, version) which taken together (hopefully) uniquely identify the project against every Maven project produced globally. This is done by convention that the project group identifier starts with a reversed domain name belonging to the owning organization (in this case `uk.ac.mmu`) followed by 'project group' id (in this case `sda`) . The artifact identifier is the name of the project (in this case `lab08`).
...
uk.ac.mmu.sda.lab08.Lab08Application     : Starting Lab08Application using Java 25.0.1 with PID 35656
uk.ac.mmu.sda.lab08.Lab08Application     : The following 1 profile is active: "default"
```
...
Now create a class inside the `uk.ac.mmu.sda.lab08` package
...
```
When you run the application again, you should see a message similar to `Hello from class uk.ac.mmu.sda.Runner01` printed in the console output.
...
Copy the `AbstractCreditCard` interface , `TestCreditCardService`, and `CheckoutService` *exactly* as described at the start of this lab inside the `uk.ac.mmu.sda.lab08` package.
...
class Runner01 implements org.springframework.boot.CommandLineRunner, org.springframework.core.Ordered {
    ...
    public void run(String... args)  {
        ...
        abstractCreditCardService.takePayment("1234-5678-9012-3456", 2028, 6, "123", 100.0d);
    }
...
```
You should see a message from Runner01 printed in the console output along with the message from the TestCreditCard service implementation.

```Text
Hello from class uk.ac.mmu.sda.lab08.Runner01
Depends on uk.ac.mmu.sda.lab08.TestCreditCardService@42d236fb
TestCreditCardService charging credit card 1234-5678-9012-3456 for £100.00
```
...
Modify Runner02 to inject an instance of CheckoutService into its constructor and call `chargeCreditCard()` in the `run()` method.
...
```Text
Hello from class uk.ac.mmu.sda.lab08.Runner02
Depends on uk.ac.mmu.sda.lab08.CheckoutService@538cd0f2 uk.ac.mmu.sda.lab08.TestCreditCardService@7a356a0d
CheckoutService charging credit card...
TestCreditCardService charging credit card 1234-5678-9012-3456 for £100.00
```
### Explanation of how this works
...
1. The `@SpringBootApplication` annotation placed on the class containing the entry point method implicitly defines a base “search package”.
2. Any methods annotated with `@Bean` within a class annotated with `@SpringBootApplication` are treated as Bean definitions by the Spring container.
3. A bean of type `CheckoutService` is registered in the DI container using an `@Bean` annotated method that returns a `CheckoutService` instance.
4. Component scanning registers that method for providing instances of `CheckoutService` requires `AbstractCreditCardService`.
...
6. The DI container calls the constructor of Runner02, creating and providing an instance of `CheckoutService`, which itself has been created with an instance of `AbstractCreditCardService`.
...
To illustrate this, try adding another class that depends on `CheckoutService` (for example an `Order` class), create another Runner class that depends on this new class, and so on.
...
Add a class called `AppConfig` to the package with the following code.
...
```Text
Hello from class uk.ac.mmu.sda.lab08.Runner01
Depends on uk.ac.mmu.sda.lab08.TestCreditCardService@7a356a0d
TestCreditCardService charging credit card 1234-5678-9012-3456 for £100.00
Hello from class uk.ac.mmu.sda.lab08.Runner02
Depends on uk.ac.mmu.sda.lab08.CheckoutService@538cd0f2 uk.ac.mmu.sda.lab08.TestCreditCardService@7a356a0d
CheckoutService charging credit card...
...
Change the scope back to singleton and run the application again to see the original behavior.
...
Create another implementation of `AbstractCreditCardService` called `RealCreditCardService` in the package.

```java
class RealCreditCardService implements AbstractCreditCardService {

    public void takePayment(String cardNumber, int expiryYear, int expiryMonth, String cv2, double amount) {
        System.out.format("%s charging credit card %s for £%.2f%n", this.getClass().getSimpleName(), cardNumber, amount);
    }
}
...
1. Apply the `@Primary` annotation to `createTest()` indicate which implementation should be used by default when there are multiple candidates.
2. Annotate this the `createTest()` Bean definition method with a specific name using the `@Bean("test")` annotation.
3. Add a second Bean definition method `createReal()` for interface `AbstractCreditCardService` that returns an instance of RealCreditCardService.
4. Annotate this second Bean definition method with a specific name using the `@Bean("real")` annotation.
5. Apply the `@Qualifier` annotation to the requirement for the `AbstractCreditCardService` interface in the `createCheckoutService()` method to specify that the `real` Bean method should be used when creating CheckoutService.
...
class AppConfig {
    ...
    CourierGateway createCourierGateway() {
        return new DefaultCourierGatewayAdapter();
    }

    @Bean
    @Scope("prototype")
    CheckoutService createCheckout(@Qualifier("real") AbstractCreditCardService abstractCreditCardService) {
        return new CheckoutService(abstractCreditCardService);
    }
}
```
...
TestCreditCardService charging credit card 1234-5678-9012-3456 for £100.00
Hello from class uk.ac.mmu.sda.lab08.Runner02
Depends on uk.ac.mmu.sda.lab08.CheckoutService@1532c619 uk.ac.mmu.sda.lab08.RealCreditCardService@46044faa
CheckoutService charging credit card...
RealCreditCardService charging credit card 1234-5678-9012-3456 for £100.00
```
### Explanation of how this works
...
You can register additional components with the Spring DI container to listen to lifecycle events and customize the behavior of the DI container.
