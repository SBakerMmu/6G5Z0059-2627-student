...
- **Project**: Maven
- **Language**: Java
- **Spring Boot**: Choose latest stable (at time of writing this was 4.1.0)
- **Project Metadata**
    - Group: uk.ac.mmu.sda
    - Artifact: lab10
    - Name: lab10
    - Package name: uk.ac.mmu.sda.lab10
    - Packaging: Jar
...
uk.ac.mmu.sda.lab10.Lab10Application         : Starting Lab10Application using Java 25.0.1
uk.ac.mmu.sda.lab10.Lab10Application         : No active profile set, falling back to 1 default profile: "default"
uk.ac.mmu.sda.lab10.Lab10Application         : Started Lab10Application in 1.13 seconds (process running for 1.694)
```
...
```
You should see the message `Hello from class uk.ac.mmu.sda.lab10.Runner01` appear in the console when you run the application.

## Creating a Shipping Cost Calculator application in Spring Boot

Copy the `applicationcode` and `infrastructure` packages from the **ShippingCostCleanArchitecture** project in Student code repository to the `uk.ac.mmu.sda.lab10` package in your Spring Boot project.
...
Therefore, we can create multiple configuration classes in the sub-packages of `uk.ac.mmu.sda.lab10` to configure each part of the application separately.

For example, you can create a configuration class for the `uk.ac.mmu.sda.lab10.applicationcode.usecase.putregion` package like this:

```Java
package uk.ac.mmu.sda.lab10.applicationcode.usecase.putregion;
...
If we have created a Use Case class for each action that a driving actor can take, then we can standardize on UseCase classes having a single method that takes a Request object and returns a Response object.
...
> ⚠ You may see the terms "input model" and "output model" used instead of request and response objects. The word "model" conflicts with the "domain model" (meaning the entities, value objects and services modeling the business domain) and so we will use request and response to mean the input and output of a Use Case.
...
```Java
package uk.ac.mmu.sda.lab10.infrastructure.driving;

import calculateshippingrequestresponse.usecase.applicationcode.lab10.sda.uk.ac.mmu.sda.Request;
import calculateshippingrequestresponse.usecase.applicationcode.lab10.sda.uk.ac.mmu.sda.Response;
import calculateshippingrequestresponse.usecase.applicationcode.lab10.sda.uk.ac.mmu.sda.Provided;
