...
There are multiple activities each week - you will probably not get everything done in the timetabled lab sessions; therefore, it is highly recommended that you start on the lab work ahead of time and use the timetabled sessions for support. Compete the labs in your own time each week to avoid falling behind.
...
```
Imagine a simple game board with the following positions:
...
Game is won when a player has lands on their HOME position and moved at least once.

One simple implementation of the game is
...
The lab task is to write Façade classes that integrates the Game and the SingleDiceShaker implements the following interface. You can add to or modify the game class as you see fit.

Your facade class should implement the following interface:
...
```
The `play()` method should return the number of moves it took to win.
...
```
The `play()` method is a command which runs the game until it is won. As a command it changes the state of objects in the façade, and the façade itself.

The `getMoves()` method is a query returns the number of moves it took to win. As a query it does not change the state of objects in the façade, or the façade.
