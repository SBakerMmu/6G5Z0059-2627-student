...
The two concrete `LoggedInState`  and `LoggedOutState` classes depend on  `Context` and override the methods in the `State` interface to handle the events that trigger state transitions.
For convenience, we will override the toString() method so that we can easily print the current state.

> You can either pass the `Context` to the constructor of the concrete State classes, or you can pass it as a parameter to the methods in the State interface. In this lab we will use the constructor approach.
...
class LoggedInState implements State {
    ...
    public void login() {
        ...
        System.out.format("%s -> %s%n", this, this);
    }

    @Override
    public void logout() {
        State nextState = new LoggedOutState(context);
        System.out.format("%s -> %s%n", this, nextState);
...
class LoggedOutState implements State {
    ...
    @Override
    public void login() {
        State nextState = new LoggedInState(context);
        System.out.format("%s -> %s%n", this, nextState);
        context.setState(nextState);
    }
    ...
    @Override
    public void logout() {
        System.out.format("%s -> %s%n", this, this);//already logged out
    }
...
```java
System.out.println("Login Logout Example");
...
System.out.println("Autoreset example");
AuthenticationContext context = new AuthenticationContext();
System.out.format("Login%n");
context.login();
